...
export async function generateMetadata({ params }) {
  try {
    ...
    return {
      ...
      alternates: { canonical: `/en/portfolio/${params.slug}` },
    };
  } catch {
...
export default async function PortfolioDetailEn({ params }) {
  let item = null;
  let settings = {};
  try {
  ...
  const wa = `https://wa.me/${settings.whatsapp_number || '201000446294'}?text=${encodeURIComponent(`I'm interested in a project like ${item.title}`)}`;
    ...
    <main className="min-h-screen bg-white">
      ...
      <article className="py-16 sm:py-20 max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-wrap gap-4 mb-10 text-xs sm:text-sm">
          {item.client_name && (
            <span className="bg-gray-100 text-gray-700 rounded-full px-4 py-2 font-bold">
              <i className="fa-solid fa-building mr-2" />Client: {item.client_name}
            </span>
          )}
          {item.project_date && (
            <span className="bg-gray-100 text-gray-700 rounded-full px-4 py-2 font-bold">
              <i className="fa-solid fa-calendar mr-2" />{item.project_date}
            </span>
          )}
          {services.map((s) => (
        ...
        <div className="premium-card-bg border border-gray-200 rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-gray-700 font-bold text-sm sm:text-base text-center sm:text-left">Want a project like this? Get in touch now.</p>
          <div className="flex gap-3">
            ...
            <a href={wa} target="_blank" rel="noopener noreferrer" className="gold-bg-gradient text-black font-black px-5 py-3 rounded-xl text-sm whitespace-nowrap flex items-center gap-2">
              <i className="fa-brands fa-whatsapp" /> Get a consultation
            </a>
          </div>
        </div>

        <div className="mt-16 pt-8 border-t border-gray-200 text-center">
          <Link href="/en/portfolio" className="text-[#8a6d1f] font-bold hover:underline">← Back to all work</Link>
        </div>
      </article>
