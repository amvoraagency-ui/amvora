  ...
  return (
    <main className="min-h-screen bg-white" dir="ltr" lang="en">
      ...
      <section className="py-16 sm:py-20 max-w-5xl mx-auto px-4 sm:px-6">
        ...
        {featured && (
          <Link href={`/en/blog/${featured.slug}`} className="block premium-card-bg border-2 border-gold/40 rounded-3xl overflow-hidden hover:border-gold transition-all shadow-xl glow-gold mb-10 group">
            <div className="grid grid-cols-1 sm:grid-cols-2">
              ...
              <div className="p-6 sm:p-8 flex flex-col justify-center">
                ...
                <p className="text-gray-400 text-xs font-mono">
                  {new Date(featured.created_at).toLocaleDateString('en-US')} · {readingTime(featured.content, 'en')}
                </p>
              </div>
            </div>
          </Link>
        ...
        {categories.length > 0 ? (
          <BlogFilter posts={rest} categories={categories} locale="en" />
        ) : (
          rest.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
              {rest.map((post) => (
                <Link key={post.id} href={`/en/blog/${post.slug}`} className="premium-card-bg border border-gray-200 rounded-3xl overflow-hidden hover:border-gold/50 transition-all shadow-lg hover-lift block">
                  ...
                  <div className="p-6">
                    <p className="text-gray-400 text-xs font-mono mb-2">{new Date(post.created_at).toLocaleDateString('en-US')} · {readingTime(post.content, 'en')}</p>
