    ...
    <main className="min-h-screen bg-white">
      ...
      <header className="border-b border-gray-200 bg-white/95 backdrop-blur-md sticky top-0 z-50 shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex justify-between items-center">
          ...
          <Link href="/blog" className="text-xs sm:text-sm font-bold text-gray-500 hover:text-gold border border-gray-200 rounded-lg px-3 py-2">
            كل المقالات
          </Link>
        </div>
      </header>
      ...
      <article className="py-16 sm:py-20 max-w-3xl mx-auto px-4 sm:px-6">
        <p className="text-gray-400 text-xs font-mono mb-3">{new Date(post.created_at).toLocaleDateString('ar-EG')}</p>
