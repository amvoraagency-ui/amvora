...
import { getAdminPasswordHash, getEnvPasswordHash, ensureSchema } from '@/lib/db';
...
export async function POST(request) {
  ...
  try {
    await ensureSchema();
    dbPasswordHash = await getAdminPasswordHash();
  } catch (err) {
    return NextResponse.json(
  ...
  if (!dbPasswordHash && !envPasswordHash) {
    return NextResponse.json(
  ...
  const matchesDb = dbPasswordHash ? await bcrypt.compare(password, dbPasswordHash) : false;
  const matchesEnv = envPasswordHash ? await bcrypt.compare(password, envPasswordHash) : false;
