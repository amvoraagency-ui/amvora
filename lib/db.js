...
export async function ensureSchema() {
  ...
  `;
  const existingPlans = await sql`SELECT COUNT(*)::int AS count FROM pricing_plans;`;
...
export async function addPricingPlan({ name, price, tagline, features, highlighted, locale, sort_order }) {
  const rows = await sql`
    INSERT INTO pricing_plans (name, price, tagline, features, highlighted, locale, sort_order)
    VALUES (${name}, ${price || null}, ${tagline || null}, ${features || null}, ${!!highlighted}, ${locale || 'ar'}, ${sort_order ?? 0})
    RETURNING *;
  `;
  return rows[0];
}
export async function updatePricingPlan(id, { name, price, tagline, features, highlighted, sort_order }) {
  const rows = await sql`
    UPDATE pricing_plans SET name = ${name}, price = ${price || null}, tagline = ${tagline || null},
      features = ${features || null}, highlighted = ${!!highlighted}, sort_order = ${sort_order ?? 0}
    WHERE id = ${id} RETURNING *;
  `;
  return rows[0];
}
